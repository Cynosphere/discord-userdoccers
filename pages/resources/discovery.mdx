...
| games ^2^                    | array[partial [game](/resources/game#game-object) object]        | Resolved game objects from `game_application_ids`                           |
...
^2^ Only included when fetched from the [Get Invite](/resources/invite#get-invite) endpoint with `with_games` set to `true`. Only includes the `id`, `name`, `icon_hash`, and `media` fields.
...
<RouteHeader method="PUT" url="/guilds/{guild.id}/discovery-categories/{discovery_category.id}" supportsBot>
...
<RouteHeader method="DELETE" url="/guilds/{guild.id}/discovery-categories/{discovery_category.id}" supportsBot>
